...
if ($Preset -eq "x86_64") {
    ...
    .\vcpkg.exe install ixwebsocket:x86-windows-static
}
...
if ($Preset -eq "x86_64") {
    ...
    msbuild remote_64.sln /p:Configuration=releasewithsymbols /p:Platform=x64 /p:VcpkgTriplet=x64-windows-static
}
else {
    cd projects/x32/windows/vs2022
    msbuild remote_32.sln /p:Configuration=releasewithsymbols /p:VcpkgTriplet=x86-windows-static
}
